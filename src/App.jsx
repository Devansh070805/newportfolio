...
function App() {
  ...
  useEffect(() => {
    ...
    const loadAssets = async () => {
      const imagePromises = ASSETS_TO_LOAD.map((src) => {
      ...
      setTimeout(() => {
        setIsLoading(false);
      }, 2000); // Wait a bit so the user sees the cool preloader
    };

    loadAssets();

    // B. Scroll Lock (The Fix)
    // When loading starts, we disable scrolling
    document.body.style.overflow = "hidden";
    window.scrollTo(0, 0);

  }, []);

  // --- 2. UNLOCK SCROLL WHEN LOADED ---
  useEffect(() => {
    if (!isLoading) {
  ...
  return (
    <>
      {/* --- A. PRELOADER (Slides Up) --- */}
      {/* Removed mode='wait' so content is visible underneath immediately */}
      <AnimatePresence>
        {isLoading && <Preloader key="loader" />}
      </AnimatePresence>

      {/* --- B. MAIN CONTENT --- */}
      {/* IMPORTANT FIX: We render this ALWAYS (no !isLoading check).
          It sits behind the preloader (z-index lower).
          When preloader slides up, this is revealed.
      */}
